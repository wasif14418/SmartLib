    ...
    public Node build() {
        ...
        svc.getTransactions().forEach(t -> {
            if (!t.isReturned() && t.daysLeft() < 0) {
                alertBody.getChildren().add(UiFactory.alertRow("⚠️",
                ...
                alertBody.getChildren().add(UiFactory.alertRow("🕐",
        ...
        if (alertBody.getChildren().isEmpty()) {
            alertBody.getChildren().add(UiFactory.alertRow("✅","All books are on time!","No overdue or due-today items.","notif-green"));
        }
